Spree::OptionValue.class_eval do
  ...
  scope :for_product, lambda { |product| select("DISTINCT #{table_name}.*").where("spree_variants.deleted_at IS NULL AND spree_option_values_variants.variant_id IN (?)", product.variant_ids).joins(:variants)
  }
end
